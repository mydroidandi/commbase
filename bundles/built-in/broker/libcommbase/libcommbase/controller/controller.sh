...
# controller.sh
# Parses messages stored in a JSON object format file, generated by either the
# STT engine or the commbase executable. If the current_request property value
# matches a control pattern, updates the control property within the JSON file.
# Subsequently, sends the updated messages request through the
# commbase-data-exchange client.
controller() {
  # The app configuration file
  source $COMMBASE_APP_DIR/config/commbase.conf

  # The function that reads the lines from the control message pattern files
  source $COMMBASE_APP_DIR/bundles/built-in/broker/libcommbase/libcommbase/routines/read_lines_from_file.sh
  ...
  # Set the control message pattern file paths from the environment config file
    ...
    control_rerun_previous_command_patterns_file=$COMMBASE_APP_DIR$CONTROL_RERUN_PREVIOUS_COMMAND_PATTERNS_FILE;
    ...
    control_rerun_previous_command_patterns=$(read_lines_from_file $control_rerun_previous_command_patterns_file);
    ...
    if [[ -n $new_message ]]; then
        ...
        elif echo "$control_rerun_previous_command_patterns" | grep -q "$new_message"; then
            # Create the JSON object
            json_data="{\"control\": \"CONTROL_STOP_PREVIOUS_COMMAND\"}";
            # Record the control message string as a JSON object
            (echo "$json_data" > $COMMBASE_APP_DIR$RESULT_MESSAGE_RECORDING_FILE);
