...
def get_server_crt_file_path():
    ...
    with open(CONFIG_FILE_PATH, "r") as f:
        for line in f:
            ...
            # Check if the variable we are looking for exists in the line
            if variable_name == "CERTS_AND_KEYS_SERVER_CRT_FILE":
                # Remove the quotes from the value of the variable
                server_crt_file = CONFIG_FILE_DIR + value.strip()[1:-1]
...
def get_server_key_file_path():
    """
    Retrieves the value of the CERTS_AND_KEYS_SERVER_KEY_FILE variable from the
    configuration file.
    ...
    with open(CONFIG_FILE_PATH, "r") as f:
        for line in f:
            ...
            # Check if the variable we are looking for exists in the line
            if variable_name == "CERTS_AND_KEYS_SERVER_KEY_FILE":
                # Remove the quotes from the value of the variable
                server_key_file = CONFIG_FILE_DIR + value.strip()[1:-1]
...
def get_cert_pem_file_path():
    """
    Retrieves the value of the CERTS_AND_KEYS_CERT_PEM_FILE variable from the
    configuration file.
    ...
    with open(CONFIG_FILE_PATH, "r") as f:
        for line in f:
            ...
            # Check if the variable we are looking for exists in the line
            if variable_name == "CERTS_AND_KEYS_CERT_PEM_FILE":
                # Remove the quotes from the value of the variable
                cert_pem_file = CONFIG_FILE_DIR + value.strip()[1:-1]

    # Check if the variable was found
    if cert_pem_file is not None:
        return cert_pem_file

    # If the variable was not found, return None
    return None


def get_key_pem_file_path():
    """
    Retrieves the value of the CERTS_AND_KEYS_KEY_PEM_FILE variable from the
    configuration file.
    ...
    with open(CONFIG_FILE_PATH, "r") as f:
        for line in f:
            ...
            # Check if the variable we are looking for exists in the line
            if variable_name == "CERTS_AND_KEYS_KEY_PEM_FILE":
                # Remove the quotes from the value of the variable
                key_pem_file = CONFIG_FILE_DIR + value.strip()[1:-1]

    # Check if the variable was found
    if key_pem_file is not None:
        return key_pem_file
