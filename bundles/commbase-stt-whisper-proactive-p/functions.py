...
def get_stt_whisper_proactive_timeout():
    """
    Reads the 'STT_WHISPER_PROACTIVE_TIMEOUT' variable from the environment
    configuration file. Returns the string value of the variables if found, or
    None if the variable is not present.
    ...
    with open(CONFIG_FILE_PATH, "r") as f:
        for line in f:
            ...
            # Check if the variable we are looking for exists in the line
            if variable_name == "STT_WHISPER_PROACTIVE_TIMEOUT":
                # Remove the quotes from the value of the variable
                stt_engine_timeout = value.strip()[1:-1]

    # Check if the variable was found
    if stt_engine_timeout is not None:
        return stt_engine_timeout
...
def get_commbase_hardware_notification_listening_start_on():
    """
    Reads the 'COMMBASE_HARDWARE_NOTIFICATION_LISTENING_START_ON' variable from
    ...
    with open(CONFIG_FILE_PATH, "r") as f:
        for line in f:
            ...
            # Check if the variable we are looking for exists in the line
            if variable_name == "COMMBASE_HARDWARE_NOTIFICATION_LISTENING_START_ON":
                # Remove the quotes from the value of the variable
...
def get_commbase_hardware_notification_listening_stop_on():
    """
    Reads the 'COMMBASE_HARDWARE_NOTIFICATION_LISTENING_STOP_ON' variable from
    the environment configuration file. Returns the string value of the
    variables if found, or None if the variable is not present.
    ...
    with open(CONFIG_FILE_PATH, "r") as f:
        for line in f:
            ...
            # Check if the variable we are looking for exists in the line
            if variable_name == "COMMBASE_HARDWARE_NOTIFICATION_LISTENING_STOP_ON":
                # Remove the quotes from the value of the variable
                commbase_hardware_notification = value.strip()[1:-1]
