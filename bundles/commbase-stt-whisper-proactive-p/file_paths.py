...
def get_commbase_hardware_command_listening_start_file():
    """
    Retrieves the value of the COMMBASE_HARDWARE_COMMAND_LISTENING_START_FILE
    ...
    with open(CONFIG_FILE_PATH, "r") as f:
        for line in f:
            ...
            # Check if the variable we are looking for exists in the line
            if variable_name == "COMMBASE_HARDWARE_COMMAND_LISTENING_START_FILE":
                # Remove the quotes from the value of the variable
                listening_start_file = CONFIG_FILE_DIR + value.strip()[1:-1]

    # Check if the variable was found
    if listening_start_file is not None:
        return listening_start_file
...
def get_commbase_hardware_command_listening_stop_file():
    """
    Retrieves the value of the COMMBASE_HARDWARE_COMMAND_LISTENING_STOP_FILE
    variable from the configuration file.
    ...
    with open(CONFIG_FILE_PATH, "r") as f:
        for line in f:
            ...
            # Check if the variable we are looking for exists in the line
            if variable_name == "COMMBASE_HARDWARE_COMMAND_LISTENING_STOP_FILE":
                # Remove the quotes from the value of the variable
                listening_stop_file = CONFIG_FILE_DIR + value.strip()[1:-1]

    # Check if the variable was found
    if listening_stop_file is not None:
        return listening_stop_file
